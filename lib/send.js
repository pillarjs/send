...
SendStream.prototype.negotiator = function(negotiator){
...
SendStream.prototype.notAcceptable = function(){
  var res = this.res;
...
SendStream.prototype.send = function(path, stat){
  ...
  // detect content type of path
  this.detect(path);

  // negotiate content
  if (!this.negotiate(this._negotiator)) {
...
/**
 * Detect content-type and encoding based on `path`.
 *
 * @param {String} path
 * @api private
 */

SendStream.prototype.detect = function(path){
  var type = mime.lookup(path);
  var charset = mime.charsets.lookup(type);
  debug('content-type %s', type);
  debug('charset %s', charset);
  this.pathtype = type;
  this.pathCharset = charset;
};
...
SendStream.prototype.negotiate = function(){
  if (!this._negotiator) return true;
  var res = this.res;
  var req = this.req;
  return this._negotiator(req, res, this.pathtype);
};
...
SendStream.prototype.setContentType = function(){
  var res = this.res;
  if (res.getHeader('Content-Type')) return;
  res.setHeader('Content-Type', this.pathtype + (this.pathCharset ? '; charset=' + this.pathCharset : ''));
};
...
SendStream.prototype.setHeader = function(stat){
  ...
  if (this._etag && !res.getHeader('ETag')) res.setHeader('ETag', utils.etag(stat));
