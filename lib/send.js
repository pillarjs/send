...
/**
 * Enable or disable a filter function.
 *
 * @param {Function} fn
 * @return {SendStream}
 * @api public
...
SendStream.prototype.filter = function(fn){
  debug('filter %s', fn);
  this._filter = fn;
  return this;
};
...
/**
 * Check if the filename fails the filter.
 *
 * @return {Boolean}
 * @api private
 */

SendStream.prototype.isFiltered = function(){
  return fn(this.path);
};
...
SendStream.prototype.pipe = function(res){
  ...
  // filter support
  if (this._filter && this.isFiltered()) return this.error(404);
