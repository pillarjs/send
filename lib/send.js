...
SendStream.prototype.index = function(paths){
  debug('index %s', paths);
  if (paths === false) {
    this._index = [];
  }
...
/**
 * Redirect to directory with appended trailing slash
 *
 * @param {String} path
 * @api private
 */

SendStream.prototype.redirectDirectory = function(path){
  path = path || this.path;
  path += '/';
  this.redirect(path);
};
...
SendStream.prototype.pipe = function(res){
  ...
  // index file support
  if (this._index && this._index.length && this.hasTrailingSlash()) {
    var candidates = this._index.slice()
      , tryCandidate = function(){
          if (!candidates.length) return self.handlePath(path);
          var index = path + candidates.shift();
          debug('checking index "%s"', path);
          fs.stat(index, function(err){
            if (err) return tryCandidate();
            self.handlePath(index);
          });
        }
    ;
    tryCandidate();
...
/**
 * Given a `path`, handles sending, errors, or redirections
 *
 * @param {String} path
 * @api public
 */

SendStream.prototype.handlePath = function(path){
  var self = this;
  debug('stat "%s"', path);
  fs.stat(path, function(err, stat){
    if (err) return self.onStatError(err);
    if (stat.isDirectory()) {
        if (!self.hasTrailingSlash(path)) {
            return self.redirectDirectory(self.path);
        }
        else {
            if (self.listeners('directory').length) return self.emit('directory', path, stat);
            res.statusCode = 304;
            res.end("Nothing to display for directory.");
        }
    }
