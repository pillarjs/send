...
SendStream.prototype.pipe = function(res){
  ...
  self.performPipe(path,self.options.fallback)

  return res;
};

/**
 * Wraps finding the file to allow graceful fallback to a default
 *
 * @param {String} path
 * @param {fallback} fallbackPath
 * @api private
 */
SendStream.prototype.performPipe = function(path,fallback) {
  var self = this
  ...
  fs.stat(path, function(err,stat) {
    if (err) {
      var notfound = ['ENOENT', 'ENAMETOOLONG', 'ENOTDIR'];
      if (~notfound.indexOf(err.code) && fallback) {
        var fallbackPath = self._root ? join(self._root,fallback) : fallback
        return self.performPipe(fallbackPath)
      } else {
