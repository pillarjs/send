...
function SendStream(req, path, options) {
  ...
  this._extensions = options.extensions !== undefined
    ? normalizeIndex(options.extensions)
    : [ ".html", ".js", ".css", ".htm" ]
  ...
  fs.stat(path, function(err, stat){
    if (err) {
      return err.code === 'ENOENT'
        ? self.sendFileWithExtension(path)
...
/**
 * Try `path` with filename extensions.
 *
 * @param {String} path
 * @api private
 */
SendStream.prototype.sendFileWithExtension = function sendFileWithExtension(path){
  var i = -1;
  var self = this;

  function next(err){
    if (++i >= self._extensions.length) {
      if (err) return self.onStatError(err);
      return self.error(404);
    }
